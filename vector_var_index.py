    ...
    for i in range(years):
        for k in range(taxbins):
            if varindex.x(i, k) != ky:
                pass_ok = False
                print("varindex.x(%d,%d) is %d not %d as it should be" %
        ...
        for i in range(years):
            for l in range(cgbins):
                if varindex.y(i, l) != ky:
                    pass_ok = False
                    print("varindex.y(%d,%d) is %d not %d as it should be" %
    ...
    for i in range(years):
        for j in range(accounts):
            if varindex.w(i, j) != ky:
                pass_ok = False
                print("varindex.w(%d,%d) is %d not %d as it should be" %
    ...
    for i in range(years + 1):  # b[] has an extra year
        for j in range(accounts):
            if varindex.b(i, j) != ky:
                pass_ok = False
                print("varindex.b(%d,%d) is %d not %d as it should be" %
                      (i, j, varindex.b(i, j), ky))
            ky += 1
    for i in range(years):
        if varindex.s(i) != ky:
            pass_ok = False
            print("varindex.s(%d) is %d not %d as it should be" %
                  (i, varindex.s(i), ky))
        ky += 1
    if accmap['aftertax'] > 0:
        for i in range(years):
            for j in range(accounts):
                if varindex.D(i, j) != ky:
                    pass_ok = False
                    print("varindex.D(%d,%d) is %d not %d as it should be" %
...
class vector_var_index:
    ...
    def __init__(self, iyears, itaxbins, icgbins, iaccounts, iaccmap):
        ...
        self.vsize = self.xcount + self.ycount + self.wcount + \
            self.bcount + self.scount + self.Dcount
